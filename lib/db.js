...
// One row per reminder email sent — powers send idempotency (skip if already
// sent this Sydney day / ISO week) and feeds the later Success-Metrics view.
...
let _db;
export function db() {
  if (!_db) _db = drizzle(neon(process.env.DATABASE_URL), { schema: { users, magicTokens, sessions, attempts, feedback, reminderLog, appConfig } });
  return _db;
}
