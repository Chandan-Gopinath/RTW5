...
function recentWeeks(now, n) {
  ...
  for (let i = n - 1; i >= 0; i--) {
    ...
    const label = p(dt.getUTCDate()) + "/" + p(dt.getUTCMonth() + 1); // compact dd/mm for the axis
    out.push({ week: key, label });
  }
  return out;
}

// The last `n` Sydney days ending today (oldest → newest), each with its dayKey
// ("YYYY-MM-DD") and a compact dd/mm axis label derived from that key.
function recentDays(now, n) {
  const out = [];
  for (let i = n - 1; i >= 0; i--) {
    const day = dayKey(new Date(now.getTime() - i * DAY));
    const [, mm, dd] = day.split("-");
    out.push({ day, label: `${dd}/${mm}` });
  }
  return out;
}
...
export function computeMetrics(users = [], attempts = [], now = new Date()) {
  ...
  // Daily trend — distinct active users per Sydney day (last 14 days).
  const usersByDay = new Map();
  for (const a of attempts) {
    const dk = dayKey(a.createdAt);
    if (!usersByDay.has(dk)) usersByDay.set(dk, new Set());
    usersByDay.get(dk).add(a.userId);
  }
  const dailyTrend = recentDays(now, 14).map((d) => ({
    ...d,
    users: usersByDay.has(d.day) ? usersByDay.get(d.day).size : 0,
  }));
